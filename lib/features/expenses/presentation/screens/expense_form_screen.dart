...
class _ExpenseFormScreenState extends ConsumerState<ExpenseFormScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (isEditing) {
      result = await notifier.updateExpense(
        id: widget.expenseId!,
      ...
      result = await notifier.createExpense(
        description: _descriptionController.text.trim(),
        amount: amount,
        date: _selectedDate,
        categoryId: _selectedCategoryId,
    ...
    if (mounted) {
      if (result != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(isEditing ? 'Gasto actualizado' : 'Gasto registrado'),
          ),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              isEditing ? 'Error al actualizar gasto' : 'Error al registrar gasto',
            ),
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    if (isEditing) {
      ...
      if (expense != null) {
        _initializeWithExpense(expense);
      }
    }
